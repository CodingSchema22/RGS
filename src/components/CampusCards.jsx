import { motion } from "framer-motion";
import campuses from "../data/campuses";
import { Link } from "react-router-dom";

export default function CampusCards() {
  return (
    <section className="relative py-20 px-6 bg-gradient-to-br from-red-50 via-white to-red-100 overflow-hidden">

      {/* Background Glow */}
      <div className="absolute top-0 left-1/2 w-[500px] h-[500px] bg-red-300 opacity-20 blur-3xl rounded-full -translate-x-1/2"></div>

      <div className="text-center mb-14">
        <h2 className="text-4xl font-bold text-gray-800">
          Our Campuses
        </h2>
        <p className="text-gray-500 mt-2">
          Explore our connected learning environments across all branches
        </p>
      </div>

      {/* Perspective added here */}
      <div className="grid md:grid-cols-3 gap-8 relative z-10 [perspective:1000px]">

        {campuses?.length > 0 ? (
          campuses.map((campus, index) => (
            <motion.div
              key={campus.id}
              initial={{ opacity: 0, y: 40 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{
                delay: index * 0.15,
                duration: 0.6,
                type: "spring",
                stiffness: 120,
              }}
              whileHover={{
                scale: 1.05,
                rotateX: 6,
                rotateY: -6,
              }}
              whileTap={{ scale: 0.97 }}
              className="group relative rounded-2xl bg-white/60 backdrop-blur-xl border border-white/40 shadow-lg hover:shadow-2xl transition-all"
            >
              {/* Link INSIDE card */}
              <Link
                to={`/campus/${campus.id}`}
                className="block p-6 h-full"
              >
                <div className="relative z-10">

                  {/* Icon */}
                  <div className="w-12 h-12 bg-red-500 text-white flex items-center justify-center rounded-xl mb-4 font-bold">
                    {campus?.name?.charAt(0) || "C"}
                  </div>

                  {/* Title */}
                  <h3 className="text-xl font-bold text-gray-800">
                    {campus?.name || "Unknown Campus"}
                  </h3>

                  {/* Location */}
                  <p className="text-gray-500 mt-1">
                    {campus?.location || "No location available"}
                  </p>

                  {/* Students */}
                  <div className="mt-5 flex items-center justify-between">
                    <span className="text-sm text-gray-600">
                      Students
                    </span>

                    <span className="px-3 py-1 bg-red-100 text-red-600 rounded-full text-sm font-semibold">
                      {campus?.students || 0}+
                    </span>
                  </div>
                </div>

                {/* Hover Glow Effect */}
                <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-red-400/10 to-transparent opacity-0 group-hover:opacity-100 transition duration-300"></div>

              </Link>
            </motion.div>
          ))
        ) : (
          <p className="text-center col-span-3 text-gray-500">
            No campuses available
          </p>
        )}

      </div>
    </section>
  );
}